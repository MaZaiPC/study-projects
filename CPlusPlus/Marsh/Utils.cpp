...
PTCHAR StrUpper(PTCHAR tcstr)
{
	int i = 0;
	TCHAR A('A'), a('a');
	while (tcstr[i] != '\0') {
		if ((tcstr[i] >= 'a' && tcstr[i] <= 'z') || (tcstr[i] >= 'а' && tcstr[i] <= 'я'))
			tcstr[i] -= A-a;
		i++;
	} // while
...
{
	tMark(text, 0, LtYellow, LtWhite,0);
} //tMark
...
{
	tMark(text, border, LtYellow, LtWhite,0);
} //tMark
...
{
	tMark(text, border, sClr, LtWhite,0);
} //tMark
