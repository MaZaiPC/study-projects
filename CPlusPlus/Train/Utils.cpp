...
//	COORD result = SBInfo.dwCursorPosition;
//
//	unsigned long tmp;
//	for (int i = 0; i < 100; i++){
//		char symbol[5];
//		sprintf(symbol, "%d", i);
//
//		if (!SetConsoleCursorPosition(hOut, result))
//			return;
//		if (!WriteConsole(hOut, symbol, strlen(symbol), &tmp, 0))
//		{
...
// Перегрузки (параметры по умолчанию)
void tMark(char* text)
{
	tMark(text, 0, LtYellow, LtWhite);
} //tMark

void tMark(char* text, int border)
{
	tMark(text, border, LtYellow, LtWhite);
} //tMark

void tMark(char* text, int border, CColor sClr)
{
	tMark(text, border, sClr, LtWhite);
} //tMark

void tMark(char* text, CColor sClr, CColor eClr)
{
	tMark(text, 0, sClr, eClr);
} //tMark
void tMark(char* text, CColor sClr)
{
	tMark(text, 0, sClr, LtWhite);
} //tMark
// Конец перегрузок
